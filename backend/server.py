...
class TaperStep(BaseModel):
    ...
    notes: Optional[str] = None  # e.g., "Then reduce"
...
class PrescriptionCreate(BaseModel):
    ...
    instructions: str
    private_doctor_notes: Optional[str] = None  # Visible only to doctor
...
# Speech-to-Text (Doctor mic input - Whisper via Emergent LLM key)
@api_router.post("/prescriptions/transcribe")
async def transcribe_audio(
    ...
    try:
        ...
        # Determine extension from filename or content type for Whisper
...
@api_router.post("/prescriptions/drug-interactions")
async def check_drug_interactions(
    ...
    if not request.medications or len(request.medications) < 1:
...
# Private Doctor Notes - history per patient (doctor-only)
@api_router.get("/prescriptions/private-notes/{client_phone}")
async def get_private_notes_history(
    client_phone: str,
    current_user: dict = Depends(get_current_user)
):
    if current_user.get('profession') != 'doctor':
...
# One-click ABHA linking for an existing prescription
@api_router.post("/prescriptions/{prescription_id}/link-abha")
async def link_prescription_to_abha(
    prescription_id: str,
    current_user: dict = Depends(get_current_user)
):
    if current_user.get('profession') != 'doctor':
    ...
    prescription = await db.prescriptions.find_one(
        {"id": prescription_id, "professional_id": current_user['id']},
        {"_id": 0}
    )
    if not prescription:
...
async def create_prescription(prescription: PrescriptionCreate, current_user: dict = Depends(get_current_user)):
    ...
    # Optional one-click ABHA linking
    ...
    # Generate prescription text for WhatsApp (patient-friendly, no private notes)
